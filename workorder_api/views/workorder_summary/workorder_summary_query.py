    ...
    with connection.cursor() as cursor:
        cursor.execute(
            """SELECT 
                ...
                FROM workorder w
                LEFT JOIN (
                    SELECT workorder_id, MIN(created_at) AS first_begin
                    FROM workorder_activity
                    WHERE activity = 'BEGIN'
                    GROUP BY workorder_id
                ) begin_activity ON begin_activity.workorder_id = w.id
                LEFT JOIN (
                    SELECT workorder_id, MAX(created_at) AS closed_at
                    FROM workorder_activity
                    WHERE activity = 'CLOSE'
                    GROUP BY workorder_id
                ) close_activity ON close_activity.workorder_id = w.id
                LEFT JOIN (
                    SELECT workorder_id, SUM(duration) AS total_duration
                    FROM workorder_timeline
                    WHERE is_delete = FALSE
                    GROUP BY workorder_id
                ) timeline_sum ON timeline_sum.workorder_id = w.id
                WHERE w.id = 1
                  AND w.tenant_id = 1
